...
The genuinely useful part is that you can extract pure Mermaid syntax from whatever you've built on the whiteboard, syntax that's admittedly a bit messier than what you'd write by hand, but usable. At the time of writing, the whiteboard only supports flowcharts.

## The AI chat feature

Mermaid Chart also has an AI chat for generating diagrams from a text prompt. My first attempt, deliberately vague, only returned a text response rather than a diagram, so I learned you need to be specific. Using one of the built-in sample prompts worked well and produced an editable flowchart quickly.
